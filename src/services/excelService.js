...
export class ExcelService{
  defaultfont = 'Arial 10 #000000';

  constructor(
   ) {}

  create(defaultFont) {
    return $JExcel.new(defaultFont);
  }
   ...
   * @returns {string}
   * @description Limit Excel sheet name to 31 characters otherwise excel file won't be created properly
   */
  limitExcelSheetName(fileName) {
      let limitedName = fileName;
      if(fileName.length > 31) {
        limitedName = fileName.substring(0,31);
      }
      return limitedName;
  }
  ...
  toArray(s) {
    ...
    if (typeof s == 'string') {
      var ss = s.split('-');
      if (ss.length < 2) return parseInt(s);
      var a = [];
      for (var i = parseInt(ss[0])||0; i <= parseInt(ss[1]); i++) {
        a.push(i);
      }
      return a;
    }
  ...
  generate(fileName, sheets) {
    ...
    var excel = $JExcel.new(this.defaultfont);
    for(var sI in sheets) {
      excel.set( {sheet:sI,value:sheets[sI].sheetTitle} );
      ...
      sheets[sI].rows.forEach((row, rI) => {
        sheets[sI].rows[rI].forEach((cell, cI) => {
          excel.set(sI, cI, rI, cell);
        });
      });

      sheets[sI].styles.forEach(style => {
        var s = excel.addStyle(style);
        var rows = this.toArray(style.row);
        var cols = this.toArray(style.col);
        rows.forEach(row => {
          cols.forEach(col => {
            excel.set(sI, col, row, style.width ? style.width : style.height ? style.height : undefined, s ? s : undefined);
          });
        });
      });
    }
  ...
  toExcelLocalTime(date) {
    return $JExcel.toExcelLocalTime(date);
  }
}

/* taken from here https://www.codeproject.com/Articles/1197349/Excel-Files-in-JavaScript-the-Simple-Way
...
(function () {
  ...
  function getCell(x, y) {
      ...
      return (row[x] ? row[x] : setV(row, x, {v:''}));
  }
  ...
  function toStyleXml(style) {
      ...
      var v = '';
      if (style.align) {
        h = align[style.align.charAt(0)];
        v = align[style.align.charAt(1)];
      }
      if (h || v || style.wrapText) {
          alignXml = "<alignment ";
          if (h) alignXml = alignXml + ' horizontal="' + h + '" ';
          if (v) alignXml = alignXml + ' vertical="' + v + '" ';
          if (style.wrapText) alignXml = alignXml + ' wrapText="' + 1 + '" ';
          alignXml = alignXml + " />";
      }
  ...
  function normalizeFont(fontDescription) {
      ...
      while (list[0] == "none") list.splice(0, 1);                               // Delete "none" that we might have
  ...
  function AddStyleToCell(cell, styles, toAdd) {
      ...
      if (!b) return;                           // If the toAdd style does NOT addthing new
  ...
  $JExcel.new = function (defaultFont) {
    ...
    excel.generate = function (callback) {
      ...
      zip.generateAsync({ type: "blob" }).then(function (content) { callback(content); }); // And generate !!!
    }
    return excel;
  }
})();